...
# TODO check if this simple inversion works, the official inversion process is more sophisticated
...
#        return box_inv
    ...
    def _tablelookup(self,table,val):
        row = 0xf & (val >>4)
        col = 0xf & val
...
# TESTED forth and back!
        ## first operation in the diffusion layer on 8-bit values
        hexlst = 0x0
        for idx in range(self._blocksize/8):
            hexlst = self._hexlst_append(hexlst, self._hexlst_getnth(state, table[idx], self._blocksize))
        return hexlst
...
#                    print "shift 120 - (col*4 + ccol)*8 = 120 - (%d*4 + %d)*8 = %d" %(col,ccol,(120-(col*4+ccol)*8))  
#                    print "vala %#.2x" % vala   
#                    print "table[%d][%d] %#.2x" % (row,ccol,table[row][ccol])  

                    arr.append(self._gfmult(vala, table[row][ccol]))
#                    print "arr [%s]\n" % ", ".join("%#.2x"% i for i in arr)  
